...
拡張機能の機能を一通り目で確かめるための手順。

**「作成物」の欄があるものは実機でコンパイルが通ることを確認済み**なので、
「サンプルが間違っているのか、拡張機能が間違っているのか」で迷わずに済む。
欄が `—` のものは**プログラム全体としては未確認**——参照するファイル
（`EMPMSTP` / `EMPDSP` など）が実機に無いため。**桁の書き方は確かめてある**
（`20260828-rpg-sample-columns`。D 仕様書だけを取り出して `CRTBNDRPG` に通した）。

> 直す前は「すべてコンパイルが通ることを確認済み」と書いていたが、
> **`SLSENT01.rpgle` と `EMPMNT01.rpgle` の D 仕様書は実機が通さない桁**だった
> （長さ欄が 33-39 桁に収まっておらず 7 桁ぶん左）。lint の指摘は真陽性で、
> サンプルの方を直した。
...
| `SLSENT01.rpgle` / `EMPMNT01.rpgle` | —（D 仕様書のみ確認） | RPG の H/F/D/C 仕様書、補完 |
