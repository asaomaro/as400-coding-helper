...
sequenceDiagram
  ...
  A->>H: 4. EVFEVENT メンバーを SQL SELECT
  A->>A: 5. @ibm/ibmi-eventf-parser で構造化 → 修正判断
  Note over A: エラーなら 1 へ（上限あり: 6 章）<br>通れば役割 2 のテストへ
```

- 全段が個別実証済みまたは既存部品（F1/F8）。未実証は「EVFEVENT の SQL 取得」と
  「eventf-parser の実出力への適用」の 2 点（7 章 #1 の確認計画）
- 実装形態は**レシピ集（skill）→ CLI に段階化**（spec 決定）。レシピは
  本 PJ の `.claude/skills/` に置き、AGENTS.md の資格情報規約（環境変数のみ）に従う
...
| # | 未検証事項 | 確認方法 | 設計への影響 |
|---|---|---|---|
| 1 | EVFEVENT メンバーの SQL 取得＋`@ibm/ibmi-eventf-parser` の実出力適用 | 実機で `CRTBNDRPG OPTION(*EVENTF)` → SELECT → パース | 自律ループ（4.1）の 4-5 段。失敗時はスプールのコンパイルリスト解析に切替 |
