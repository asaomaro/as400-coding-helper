...
> **v6 も 7.3 で使える（2026-08-31 訂正）。** 下の「使えない」は**回避策を探していなかった**
> ための誤りだった。`RUCRTRPG … TGTCCSID(0)` を渡せば通る——`CRTTST.RPGLE` の
> `serializeTgtCcsid` が `if (tgtCcsid = 0) return '';` で、キーワードごと消えるため
> （v5 / v6 とも同じ実装）。対照つきで実機確認済み: 既定は落ち、`TGTCCSID(0)` は通った。
>
> **V2 API（`assertEqual`）は v5 以降にしかない。** 設計書 4.2 が求める書き方なので、
> **新規に入れるなら v6 を選ぶ。**

| 版 | `RSTLIB` | 7.3 で使えるか | V2 API |
|---|---|---|---|
| **v6.0.2.r（最新・推奨）** | 通る | **○**（`TGTCCSID(0)` が要る） | **有り** |
| v5.1.0.r | 通る見込み | 同上（未検証） | 有り |
| v4.0.3.r | 通る | ○（`TGTCCSID` 自体が無い） | **無し** |

以下は「既定のままだと落ちる」理由の記録。
...
> 「復元できた＝使える」と読まないこと。
>
> **ただし「使えない」とも限らない。** ここで止まらず回避策を探すべきだった
> ——`TGTCCSID(0)` で通る。**症状を見て結論を出す前に、逃げ道の有無を確かめる。**
