...
// I/O 仕様書は「レコード識別行」と「フィールド記述行」で桁の意味が異なり、
// 1つの定義では表せない。行タイプごとに定義を分けるまで検査対象外とする。
const UNSUPPORTED = {
  "I-SPEC": "レコード識別行とフィールド記述行で桁の意味が異なる（定義の分割が必要）",
  "O-SPEC": "レコード識別行とフィールド記述行で桁の意味が異なる（定義の分割が必要）",
