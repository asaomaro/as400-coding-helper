...
const LAYOUTS = {
  ...
  "O-SPEC-REC-PGM": {
    ...
    fields: [
      ...
      // 18-20 桁は用途が 2 つあり、原典も同じ桁を別々に説明している
      // （18 桁目「Fetch Overflow/Release」と 18-20 桁「Record Addition/Deletion」）。
      // 両立しないので入力欄も 1 つにする。別々の欄にすると、片方に入力した値が
      // もう片方を壊し、読み戻しも曖昧になる。
      ["OVFADD", "フェッチ・オーバーフロー／レコードの追加・削除", 18, 20,
       "印刷装置ファイルでは 18 桁目に F=フェッチ・オーバーフロー、R=解放。データベース・ファイルでは 18-20 桁に ADD=追加、DEL=削除。どちらか一方だけを書く。"],
