...
> ### 桁位置の正本は `*-SPEC-layout.html`
> 各仕様書の全桁を列挙しているのは `docs/origin/ilerpg/<X>-SPEC-layout.html`
> （「従来型の〜ステートメント」のトピック）。仕様書の入口ページには桁の一覧が
> 無いため、桁照合はこちらを使う。本文は「N 桁目 (ラベル)」の形で列挙されている。
>
> **範囲の一致だけで判定してはならない。** 項目名と原典のラベルを対応させて
> 突き合わせること。例: F仕様書の `FILEFMT` は 19 桁目にあったが、19 桁目の
> 原典ラベルは「ファイルの終わり」で、「ファイル形式」は 22 桁目だった。
> 範囲一致だけでは OK と誤判定する。
>
> 往復検証（`npm run verify:roundtrip`）は「読み書きで同じ桁を使う」ことしか
> 見ないため、**桁が誤っていても通る**。桁の正誤は原典照合でしか確定できない。
>
> #### 既知の不一致（未修正）
> 原典照合の結果、以下が原典と食い違っている。修正は別作業。
> - F-SPEC: `FILEFMT`(19→22) `RECLEN`(20-23→23-27) `LIMITS`(24-27→28)
>   `RECADDR`(28→34) `FILEORG`(29-33→35) `DEVICE`(34-42→36-42) の6件。
>   さらに「ファイルの終わり(19)」「ファイルの追加(20)」「順序(21)」
>   「予約済み(43)」が定義に存在しない。
> - D-SPEC: `NAME`(8-21→7-21)
> - 全仕様書: `COMMENT`(81-130→81-100。原典「仕様書の注記部分は 81 から 100 桁目」)
