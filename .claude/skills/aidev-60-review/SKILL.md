...
2. 差分を以下の観点で点検する。
   ...
   - **到達可能性 / 配線**: 追加した成果物（定義 JSON・リソース・コマンド等）が、それを消費する
     コードパスに登録され、**実際に利用者の操作から到達できる**か。**消費経路を名指しで辿り、
     辿った根拠を review.md に残す**（例: 定義 → `positionResolver` → キーバインドの発火条件）。
     表示系を見て消費系を確認したことにしない（過去に取り違えて死蔵を見逃している）。
     同じ概念集合が複数箇所で列挙されていないか（列挙の同期漏れ）も併せて見る。
