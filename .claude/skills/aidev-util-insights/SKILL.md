...
- `.aidev/works/*/metrics.yml`：工程時間・手戻り回数・差し戻し回数・リードタイム・件数指標。
- `.aidev/works/*/review.md`：レビュー指摘の内容（再発パターン分析の主材料）。
- `.aidev/works/*/decisions.md`：繰り返される設計逸脱。
- `.aidev/works/*/retro.md`：過去の per-work 改善提案（再発・未対応の把握）。
...
2. 各 works の入力を読み、横断で集計する。重い場合は works 単位の読み取りを委譲する。
3. 次の観点で傾向を抽出する。
   - **レビュー指摘の再発**：同種・同観点の指摘が複数作業で繰り返されていないか。
   - **ボトルネック工程**：手戻り回数・差し戻し・経過時間が突出する工程はどれか。
