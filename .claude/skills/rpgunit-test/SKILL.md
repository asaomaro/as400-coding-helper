...
**この印は道具が検査する。** `run-rpgunit.mjs` が印を読み、**印が無いテスト**と
**印だけあって根拠の行が無いもの**を欠陥として報告する（単語だけの `VERIFICATION` は
名乗りの偽装なので通らない）。既定は警告で実行は続き、**レポートに載る**。
`--require-oracle` を付けると**走らせずに失敗**する（CI 用。出所不明のまま走ると
緑のレポートが出来てしまうため）。

```
▸ オラクル 検証 2 / 特性化 0            ← 揃っているとき

⚠ オラクルの印がありません（skill rpgunit-test §0.2）:
    ✗ TESTTAX  出所不明（VERIFICATION / CHARACTERIZATION の印が無い）
```

レポートには**テストごとの出所**と内訳（`検証 2 / 特性化 1 / 不明 0`）が出る。
特性化テストを「網羅した」という報告に混ぜない（0.4）。

**実機が報告したテストと突き合わせる。** ソースから読めなかった手続きがあれば
出所不明として扱う。読み落としがあると検品ごと素通りするため
（実際、**継続名前行**——名前が 15 桁に収まらず `...` で次行に続く形——を
読めておらず、長い名前のテストが 1 件も見えていなかった）。
...
| 検品 | 何を確かめるか | コマンド |
|---|---|---|
| **出所** | 期待値がどこから来たか名乗っているか | `run-rpgunit.mjs`（既定で警告 / `--require-oracle` で失敗） |
...
含まれるもの: 判定・集計・**実行条件**（ソースタイプ／バインド先／実行順／独立性）・
テストケースの一覧・**失敗の詳細**（message と Callstack）・**環境**
（ライブラリー・リスト／OS／iRPGUnit のバージョン。XML の `<properties>` 由来）。

**失敗しても独立性が食い違ってもレポートは出る**（失敗したときこそ要るため）。
終了コードは従来どおり。
