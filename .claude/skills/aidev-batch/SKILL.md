---
name: aidev-batch
description: AI開発ワークフローのバッチ駆動ユーティリティ。バックログ（チェックリスト）の未処理項目を順に autonomous モードの aidev で処理し、PRにまとめる。「バックログを消化して」「まとめて定義を作って」「batch」などと言われたとき、または /loop・/schedule から起動されたときに使用する。
...
- **依存**: 順序の正は生成する work の `state.yml` `dependsOn`（`protocol.md`「2.7」）。
  ...
  `(needs: <slug/#N>)` で表してよい。batch は work 作成時にこれを `dependsOn` へ転記し、未充足なら保留＝次項目へ。
...
3. **1回の処理件数上限**を決める（既定 3、必要なら確認）。上限まで、未チェックの**先頭から**処理：
   - その項目テキストを **task** として `mode: autonomous` の aidev を実行する
...
- **依存順を尊重する**: 項目に前提があれば、作成する work の `state.yml` `dependsOn` に設定する。
