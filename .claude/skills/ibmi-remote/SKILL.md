...
##### **スプールは日本語機では読めない**（5250 で実機確認）

XML だけの問題ではない。**スプールの報告は、日本語 CCSID の表示装置では
ASCII の部分まで化ける**（`DSPSPLF` を 5250 で開いて確認）。

```
実際の画面:  *** Tｵﾍﾎﾍ ﾅｶ MSGTST ***        ← *** Tests of MSGTST ***
             Dｱﾎｵ  : 2026-08-30            ← Date
             ASCII ｶｱｹﾂﾏﾈｵ ﾎｵﾒﾎ            ← ASCII failure text
             ･a･eｨX､ﾉ｣ﾒ｣ｯ｣ｾ｣ｰ｣F            ← 日本語のメッセージ
```

**小文字がカタカナになる**のが特徴で、ラテン系 EBCDIC のバイトを
カタカナ系 CCSID の装置で描いたときの典型。RPGUnit は他社機（savf の
`System name` は `GFD400`）でビルドされており、**プログラム内のリテラルが
ラテン系のコードページで焼かれている**と考えると全部説明がつく。

- **対照は同じ画面にある。** サインオン画面と IBM i メインメニューは
  同じセッションで日本語が正しく出る。つまりエミュレーター側ではなく、
  **スプールのデータだけが他と違うコードページ**にある。
- **`CHGJOB CCSID(37)` では直らない**（表示は装置の文字セットで決まる）。
- **プログラムから読む分には困らない。** `readSpooledPages` をラテン系で
  デコードすると ASCII は正しく読める（日本語は元々失われている）。

→ **日本語機で報告を人が読むなら、スプールは当てにしない。**
...
- スプールの文字化けを**直す手段**（`CHGSPLFA` の `CHRID` 等で救えるか）。
  化けること自体は 7.4 で確定済み。
