...
- [ ] **キーワード欄の編集（L3）** — `DSPATR` / `COLOR` / `EDTCDE` 等の追加・削除・引数の変更。
  読むところまでは済んでいる。書き戻しでは
  **1 つのキーワードが 45-80 桁（36 桁）に収まらない場合**に `-` で折る必要がある
  （規則は実機で確定済み。上記 work の `research.md` F1）。
  いまは継続にまたがるキーワード欄の書き換えを `keyword-continuation` で拒否している。

- [ ] **継続の書き出し（折り返し）** — 長いキーワードを `-` で折って書き出す。
  読む側は済んでいるので、**折った結果を読み直すと元に戻ること**を検査で固定できる。
