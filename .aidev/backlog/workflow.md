...
- [ ] RPG III の残りの数値欄を実機で確定する — F 仕様（`BLOCKLEN` / `RECLEN` /
      `RECADDRLEN` / `KEYSTART`）・O 仕様（`SPACEBEFORE` / `SPACEAFTER` / `SKIPBEFORE` /
      `SKIPAFTER` / `ENDPOS`）・E 仕様・L 仕様。C / I 仕様と同じ手法で確かめられる
      （`20260828-rpg3-numeric-columns/verify/` の probe が雛形）。
      **対照を先に通すこと**——RPG III は最小プログラムを組むのが難しく、
      対照が落ちたまま進むと桁ではない失敗を桁の規則として記録する。
