...
function extractRpgInitialValues(
  resolved: ResolvedPosition,
  definition: PrompterDefinition
): Values {
...
function extractRpgDspecValues(
  text: string,
  definition: PrompterDefinition
): Values {
  const values: Values = {};
...
function extractRpgCspecValues(
  text: string,
  definition: PrompterDefinition
): Values {
  const values: Values = {};
...
function extractRpgCnewValues(
  ...
  const afterOpcode =
    trimmedTail.slice(trimmedTail.indexOf(opcode) + opcode.length).trim();
...
function extractClInitialValues(
  resolved: ResolvedPosition,
  definition: PrompterDefinition
): Values {
  const document = resolved.document as vscode.TextDocument;
  ...
  return mapParsedCommandToValues(definition, parsed);
}
