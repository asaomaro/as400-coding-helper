 ...
 */
export function buildCommandHelpText(definition: PrompterDefinition): string {
  ...
  if (definition.restrictions?.length) {
    sections.push(
      ["制約事項:", ...definition.restrictions.map(text => `  ・${text}`)].join("\n")
    );
  }
    ...
    sections.push(
      [
        ...
        ...definition.constraints.map(constraint => {
          const label =
            constraint.kind === "exclusive" ? "同時指定不可" : "一緒に指定が必要";
  ...
  if (positional.length > 0) {
    sections.push(
      ["定位置指定順:", ...positional.map(p => `  ${p.positional}. ${p.name} (${p.description})`)].join("\n")
    );
  }
 ...
 */
export function buildParameterHelpText(parameter: ParameterDefinition): string {
  const sections: string[] = [];
...
): { positional: number; name: string; description: string }[] {
  return parameters
    .filter(
      (parameter): parameter is ParameterDefinition & { positional: number } =>
        typeof parameter.positional === "number"
    )
    .map(parameter => ({
