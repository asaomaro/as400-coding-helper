 ...
 * プロンプターの入力値に戻すために使う。
 *
 * プロンプターは「開く → 確定 → 再度開く → 確定」で同じ結果になる必要があるため、
 * ここでの解析は applyChanges.ts の生成と対になっていなければならない。
 */

export interface ParsedClCommand {
  readonly label?: string;
  readonly keyword: string;
  /** キーワード指定 `NAME(値)` の、括弧の内側の生テキスト。 */
 ...
 * 引用符の中の記号は継続ではないので除く。
 */
export function isContinuedLine(text: string): boolean {
  const trimmed = stripComments(text).trimEnd();
  const marker = trimmed.slice(-1);
  return (
    (marker === "+" || marker === "-") && !isInsideQuotes(trimmed.slice(0, -1))
  );
}

export function joinContinuationLines(lines: readonly string[]): string {
  let result = "";
...
function isCommentBoundary(previous: string | undefined): boolean {
 ...
 */
export function extractComments(lines: readonly string[]): string[] {
  ...
  for (const line of lines) {
    ...
    for (let i = 0; i < line.length; i += 1) {
      ...
      if (char === "/" && line[i + 1] === "*" && isCommentBoundary(line[i - 1])) {
        ...
        if (body.length > 0) comments.push(body);
        if (close === -1) break;
        i = close + 1;
      }
    }
...
export function parseClCommand(text: string): ParsedClCommand | undefined {
...
function unwrapOuterParens(text: string): string {
  ...
  for (let i = 0; i < trimmed.length; i += 1) {
    ...
    if (trimmed[i] === ")") {
      depth -= 1;
      if (depth === 0) return i === trimmed.length - 1 ? trimmed.slice(1, -1).trim() : trimmed;
    }
  }
  return trimmed;
}

const leafOf = (parameter: ParameterDefinition): boolean =>
 ...
 */
export function mapParsedCommandToValues(
  definition: PrompterDefinition,
  parsed: ParsedClCommand
