...
export type RuleId =
  ...
  /**
   * 既定 ON。**1 行 1 桁**に項目を置いた。原典（表示装置ファイルの `桁数 (30 - 34 桁目)`）:
   * > フィールドは、表示画面の最初の桁を占めることはできません。
   * > 最初の桁は属性文字のために予約されています。**例えば 24 x 80 の画面で…
   * > 39 - 41 桁目 (行) に 1 を指定し、42 - 44 桁目 (桁) に 1 を指定した**とすると…
   * > したがってこの指定は無効です。
   *
   * **他の行の 1 桁目は置ける**（属性文字は前の行の 80 桁目に入る）。実機で確認済み。
   */
  | "layout-column-one-reserved"
  /** 既定 ON。DSPSIZ の書式・値が不正。原典の有効値は 24x80 と 27x132 だけ。 */
  | "layout-invalid-screen-size"
  /**
   * 既定 ON。**画面サイズ条件名が 2 次画面サイズを指していない。**
   *
   * 実機はこの形をコンパイルしない（`CPF7311`）。8 通りを実機で確かめて確定した
   * ——「DSPSIZ に宣言してあればよい」ではなく、**2 次を指していること**が条件。
   * 項目自身の行が 1 次の位置を与えるので、1 次を条件にした指定は矛盾する。
   */
  | "layout-invalid-screen-size-condition"
  /**
   * 既定 ON。**条件を付けられないキーワードに条件標識が付いている。**
   * 原典（各キーワードの詳細ページ）:
   * > オプション標識は、このキーワードでは無効です。
   *
   * 実機はこの形をコンパイルしない（`CPF7311`。`EDTCDE` / `EDTWRD` / `CHECK` で確認済み）。
   * 可否は原典から生成した表で引く（`dds-conditioning.json`）。
