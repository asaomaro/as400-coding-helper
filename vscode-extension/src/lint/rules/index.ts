...
export const RULE_SPECS: readonly RuleSpec[] = [
  ...
  },
  {
    ...
    description:
      ...
      "その印が付くのは、実機で全空間（1 文字なら 37 通り）を試して受理集合が" +
      "定義と完全一致した欄だけ（DDS の 7 欄。物理/論理の 17・35・38 桁、" +
      "表示装置の 17・35・38 桁、印刷装置の 35 桁）。確かめていない欄や" +
      "一致しなかった欄は `restricted: false` で、列挙は候補にすぎないので咎めない。",
    positional: true
  },
  // --- レイアウト（ファイル単位）---
  ...LAYOUT_RULES.map(
