import { ddsField, ddsReplaceField } from "../ddsLayout";
...
export function writeBackPosition(request: WriteBackRequest): string {
  ...
  line = ddsReplaceField(line, DDS_POSITION_ROW, format(request.row, 3));
  line = ddsReplaceField(line, DDS_POSITION_COLUMN, format(request.column, 3));
