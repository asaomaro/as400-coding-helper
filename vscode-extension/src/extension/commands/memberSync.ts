...
import { deriveSourceType, resolveMemberTarget } from "../../sync/memberTarget";
...
async function runTransfer(
  ...
  const resolution = resolveMemberTarget(vscode.workspace.asRelativePath(document.uri, false));
  if (!resolution.ok) {
  ...
  try {
    ...
    if (direction === "upload") {
      ...
      const sourceType = deriveSourceType(target.extension);
...
function describeResolveFailure(reason: "pathShape" | "memberName" | "textDescription"): string {
  switch (reason) {
    case "pathShape":
